...
class InputConnector:
	...
	def upload_IPE_to_bucket(self, brand, directory, IPE, isProcessed, log = True):
	...
	def download_brand_training_input_post_entities(self, brand, isProcessed = None):
		...
		return self.download_brand_post_entities(self.constants.INPUT_CONTAINER_NAME, brand, path, isProcessed = isProcessed)
	...
	def download_brand_post_entities(self, brand, prefix, isProcessed = None):
		...
		logs = InputLogEntries(log_entries = self.retreive_log_entities(container_name, prefix))
		if(processing_status_filter != None):
			unproccessed_entries = logs.GetLogs(isProcessed = isProcessed)
