...
class InputConnector:
	...
	def download_brand_post_entities(self, brand, prefix, isProcessed = None):
		...
		logs = self.retreive_log_entities(container_name, prefix, isProcessed = isProcessed)
		for log in logs:
	...
	def retreive_log_entities(self, container_name, path, isProcessed = None):
		container = self._input_container()
        if(isProcessed == None):
            filter = None
        else:
        ...
        self.nfs_controller.retreive_log_entities(container, path, filter = filter)
